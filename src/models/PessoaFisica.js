...
class PessoaFisica {
  static async create(data) {
    ...
    const { rows: pessoaFisica } = await client.query(
  ...
  static async findAll() {
    const client = await pool.connect();
    const { rows: pessoasFisicas } = await client.query('SELECT * FROM pessoa_fisica');
    await client.release();
  ...
  static async findById(pessoaFisicaId) {
    const client = await pool.connect();
    const { rows: pessoaFisica } = await client.query(
      'SELECT * FROM pessoa_fisica WHERE pessoa_fisica.cpf = $1',
      [pessoaFisicaId],
    );
    await client.release();
    return pessoaFisica;
  ...
  static async findByIdAndUpdate(pessoaFisicaId, data) {
    const client = await pool.connect();
    const { rows: pessoaFisica } = await client.query(
      'SELECT * FROM pessoa_fisica WHERE cpf = $1',
      [pessoaFisicaId],
    );
    if (pessoaFisica) {
      const {
  ...
  static async findByIdAndDelete(pessoaFisicaId) {
    ...
    if (pessoaFisica) {
      await client.query('DELETE FROM pessoa_fisica WHERE cpf = $1', [pessoaFisicaId]);
      return pessoaFisica;
    }
    return pessoaFisica;
  }

...
class PessoaJuridica {
  static async create(data) {
    const { cnpj, nomeFantasia, naturezaJuridica, usuarioId } = data;
  ...
  static async findAll() {
    const client = await pool.connect();
    const { rows: pessoasJuridicas } = await client.query(
  ...
  static async findById(pessoaJuridicaId) {
    ...
    const {
      rows: pessoaJuridica
    } = await client.query("SELECT * FROM pessoa_juridica WHERE pj_id = $1", [
      pessoaJuridicaId
    ]);
    await client.release();
    return pessoaJuridica;
  ...
  static async findByIdAndDelete(pessoaJuridicaId) {
    ...
    if (pessoaJuridica) {
      await client.query("DELETE FROM pessoa_juridica WHERE pj_id = $1", [
        pessoaJuridicaId
      ]);
      return pessoaJuridica;
    }
